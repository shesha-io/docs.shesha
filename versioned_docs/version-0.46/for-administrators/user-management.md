...
`UserAppService` (`Shesha.Users.UserAppService`) is the application service behind the Users screen and is the class you would typically call from your own services. It exposes:
...
:::info Security-question and OTP-based password reset
As of this release, `UserAppService` also exposes a self-service password reset flow that does not go through an administrator: `GetUserPasswordResetOptionsAsync`, `SendSmsOtpAsync`, `SendEmailLinkAsync`, `ValidateResetCodeAsync`, `ValidateSecurityQuestionsAsync`, and `ResetPasswordUsingTokenAsync`. Which options are offered to a given user depend on their `SupportedPasswordResetMethods` and the security questions they have configured. This flow is not covered in detail on this page - see the framework's `Shesha.Users.UserAppService` source and the security settings documentation for the full picture.
:::

    ...
    public class M20231124085300 : Migration
    {
        public override void Up()
        {
            Alter.Table("Core_Persons")
                ...
                .AddColumn("Mem_IdDocumentId").AsGuid().Nullable()
                    .ForeignKey("fk_core_persons_mem_id_document_id", "frwk", "stored_files", "id").Indexed()
...
`AddForeignKeyColumn` is a Shesha extension over FluentMigrator that creates the column and its foreign key constraint in a single call. It does not take a schema, so it only works for tables in the default schema. Framework tables such as `frwk.stored_files` live in the `frwk` schema, so the column above is added with plain FluentMigrator and an explicit schema-qualified `ForeignKey`. For more migration options see the [FluentMigrator docs](https://fluentmigrator.github.io/index.html).
:::
...
See [CRUD APIs](../../../back-end-basics/crud-apis.md) for details on what endpoints Shesha exposes for each entity and how to customise them.
:::
...
try {
  const resp = await actions.callApi.post(PATH, data);
  ...
  actions.showMessage.error(`Failed to create member: ${e}`, 10);
  console.error(e);
  throw e;
}
```
...
For more on the tab component, see [the tab component reference](../../../front-end-basics/form-components/Layouts/tabs.md).
...
Drag in a **File** component for the ID document (see the [File component reference](../../../front-end-basics/form-components/Entity-References/files.md)):
...
| Option | When to use |
|---|---|
| `Autocomplete` | When users will search by typing. See [Autocomplete](../../../front-end-basics/form-components/Advanced/autocomplete.md). |
...
| `Sub Form` | When you want to embed a full form for the related entity. See [Sub Form](../../../front-end-basics/form-components/Layouts/subform.md). |

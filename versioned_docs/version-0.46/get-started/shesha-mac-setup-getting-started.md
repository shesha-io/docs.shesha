...
- **.NET 10 SDK and Runtime** - for the backend and for SQL-Package.
- **Node.js 24** (with npm 11 or later) - for the frontend.
...
If you do not already have it, download the Arm64 (Apple Silicon) or x64 (Intel) version from the [.NET 10 download page](https://dotnet.microsoft.com/en-us/download/dotnet/10.0).

### 2.2 Install SQL-Package

:::warning Pin the SqlPackage version
Install version `162.4.92` specifically. Installing without a version pulls the latest, which silently fails.
:::

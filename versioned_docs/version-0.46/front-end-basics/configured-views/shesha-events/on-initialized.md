...
| `form` | The form instance, including `form.mode`, `form.arguments`, `form.state`, and `form.setFieldsValue` |
...
| `page` | The current page: `page.state` for data shared by every component on the page, and `page.location` |
| `application` | Application-wide services and `application.state`, shared across the whole application |
| `user` | The signed-in user and their permission checks |
| `storage` | Browser local and session storage |
| `actions` | Calling APIs (`actions.callApi`), showing messages (`actions.showMessage`), and opening dialogs |
| `utils` | Helpers: `utils.moment` for dates, `utils.saveAs` to save a file, `utils.modal` for dialogs |
...
page.state.currentRecordId = query?.id;
```

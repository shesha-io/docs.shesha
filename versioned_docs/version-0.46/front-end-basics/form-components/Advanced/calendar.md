...
A unique, required name for this Calendar component. Shesha uses this to identify the component, particularly when more than one Calendar is used on the same form.
...
**Form type to use:** Edit Form - use when the calendar's starting view should follow a date already on the record.
...
**Form type to use:** Edit Form - use when the calendar's ending view should follow a date already on the record.

**Example - Set the end date 30 days after the start date:**

```javascript
return utils.moment(data.contractStartDate).add(30, 'days').toISOString();
```
...
The rest of this section describes the settings available for each individual layer, opened from the Layer Selector Settings Modal.
...
The minimum width the calendar is allowed to shrink to.

#### **Max Width** `string`

The maximum width the calendar is allowed to grow to.
...
The minimum height the calendar is allowed to shrink to.

#### **Max Height** `string`

The maximum height the calendar is allowed to grow to.

...
Events are JavaScript handlers that run when the user types in the field, focuses it, or picks an address from the dropdown. They give you a chance to react to the user's selection and pull values (such as latitude and longitude) onto the form.
...
Fires when the user clicks into or tabs into the address input. Use it for lightweight reactions such as highlighting related fields or clearing a stale error message before the user starts typing.

...
Text displayed when the component has no value and is in read-only mode. Use it to say "Not linked" rather than leaving an empty gap on a details view.
...
The width of the picker's own selection dialog. Pick one of the presets, or type your own value directly into the field.
...
:::tip
You can type any CSS width into this field, for example `500px` or `30em`. Without a unit, Shesha uses pixels.
:::
...
| Option | Behaviour |
|---|---|
| `Default` | Shesha adds standard OK and Cancel buttons that submit the dialog form. |
...
Shown only when Buttons Type is `Custom`. Add and configure the buttons to show in the dialog footer using the button group builder.

---

#### **New Record Dialog Width** `string`

The width of the create-record dialog, separate from the picker's own Dialog Width above. Pick a preset or type any CSS width.
...
The form cannot be submitted while no record has been picked.

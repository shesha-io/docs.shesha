...
:::note
Shesha always includes `id` in the request even if you leave it out of the Properties list.
:::
...
The most important thing to return here is an `id` field. Shesha uses this ID to know which record to fetch from the backend. For example, returning `{ id: '123' }` causes Shesha to call the endpoint with `?id=123`. If your Query Params script doesn't return an `id`, Shesha falls back to the `id` already present on the subform's current value before giving up and treating the ID as empty.
...
:::warning
If the resolved `id` ends up empty, null, or just whitespace, the subform clears its own fields and skips the API call entirely. Always make sure your Query Params script (or the parent form's data) actually has an `id` to fall back on, or the subform will render empty.
:::
...
The same variables are available as in Query Params: `data`, `page.state`, `queryParams`.
...
The same variables are available: `data`, `page.state`, `queryParams`.
...
**On Created** fires after the **Post form data** action completes successfully. The record has been created on the backend by the time this runs. The event exposes the created record as `submittedValue`, alongside the standard `data`, `page.state`, and `actions.showMessage` variables.

**Example - Show a success message after creating a record:**
```js
actions.showMessage.success('Address saved successfully.');
```
...
actions.showMessage.info(`Created record with ID: ${newId}`);
```

### On Updated

**On Updated** fires after the **Update form data** action completes successfully. The record has been updated on the backend by the time this runs. The event exposes the updated record as `response`, alongside the standard `data`, `page.state`, and `actions.showMessage` variables.

**Example - Show a success message after updating a record:**
```js
actions.showMessage.success('Address updated successfully.');
```
...
Both accept values between 0 and 24, and both are blank by default: when you leave them unset, the subform falls back to whatever the inner form's own Form Settings specify, and finally to Shesha's default of `6` for Label Col and `18` for Wrapper Col if neither is set. Setting Label Col to `0` hides labels entirely.

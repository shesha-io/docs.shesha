---
sidebar_label: Creating a New Front End Application
---
...
- [Overview](#overview)
- [Step 1: Duplicate the Frontend Folder](#step-1-duplicate-the-frontend-folder)
...
:::note
Shesha does not currently ship a generator or CLI command for scaffolding a new frontend application. Duplicating the existing frontend folder, as described below, is a workaround that produces a working second Next.js app - it isn't an officially packaged workflow, so treat each step as something to verify against your own project structure.
:::
...
The Shesha starter's frontend project lives in a folder named `frontend` (in the `shesha-starter` template this is `frontend/`, containing `src/app`, `package.json`, and so on). Navigate to your project root directory and duplicate it:

```bash
# In your project root directory
cp -r frontend publicportal
```
...
:::tip
Choose a meaningful name for your new application folder (e.g., `customerportal`, `publicportal`, `vendorportal`) that reflects its purpose.
:::
...
:::tip Available Shesha Form Configurations
...
These constants are exported from `@shesha-io/reactjs` (`src/components/mainLayout/constant.ts`).
:::
...
:::tip
These settings are optional and only need to be changed if you want to customize the look and feel of your new portal to differentiate it from the admin portal.
:::

:::note
To use custom forms, create them in the Forms Designer first, then update these constants to reference your custom form using the format: `{module: 'YourModule', name: 'form-name'}`.
:::
...
The `FrontEndApp` entity (`Shesha.Domain.FrontEndApp`) is mapped to the schema-qualified, snake_case table `frwk.front_end_apps` (`[Table("front_end_apps", Schema = "frwk")]`, `[SnakeCaseNaming]`). Here's a migration template that inserts a new row into it:
...
:::note
This raw-SQL example targets SQL Server only (`NEWID()`). If your project also supports PostgreSQL, branch the statement with `IfDatabase("SqlServer")` / `IfDatabase("PostgreSql")` (using `gen_random_uuid()` for the id on PostgreSQL), as the framework's own migrations that seed this table do.
:::
...
:::warning
The migration will run automatically when the backend starts. Ensure your database connection string is correctly configured in `appsettings.json`.
:::
...
### 4.1 Update the ShaApplicationProvider
  ...
  return (
    <GlobalStateProvider>
      ...
      <ShaApplicationProvider
        backendUrl={backendUrl}
        router={nextRouter}
        applicationKey={"sample-publicportal"}  {/* ADD THIS LINE */}
        noAuth={nextRouter.path?.includes('/no-auth')}
      >
        <StoredFilesProvider baseUrl={backendUrl} ownerId={""} ownerType={""}>
          {children}
...
:::warning
The `applicationKey` property value must match the `app_key` value you used in the backend migration (e.g., `'sample-publicportal'`). Without this, the application will not be properly identified in the Shesha system. `applicationKey` is a valid, optional prop of `ShaApplicationProvider` (`IShaApplicationProviderProps`, `shesha-reactjs/src/providers/sheshaApplication/index.tsx`).
:::
...
:::tip
If you don't see the record, ensure the migration ran successfully by checking the backend logs.
:::
...
1. Duplicating the `frontend` folder to `publicportal`
...
- [ ] Verify database record exists in the `frwk.front_end_apps` table

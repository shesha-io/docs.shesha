...
:::note
`swr` is already a dependency of `@shesha-io/reactjs` itself (currently pinned to `^2.2.5`). If your project's `package.json` doesn't list it directly, install a matching major version, e.g. `npm i swr@^2.2.5`, rather than the older `2.1.5` previously referenced here.
:::

In the directory `frontend\src\components\templates`, create the following files:
...
2. In this directory, `\frontend\utils`, create a `configurableActions.ts` file with the following:
...
:::note
`INavigateActoinArguments` is spelled exactly this way (with the transposed "oi") in the framework's own source - it isn't a typo introduced by this guide.
:::
...
In this directory, `frontend\src\components\templates\dynamic-templates`, create an `enquiry-initiate.tsx` file with the following:
...
:::warning `configurationItemMode` no longer exists
`useAppConfigurator()` does not return a `configurationItemMode` field. It returns `mode` (the application's current `ApplicationMode`, e.g. `'live'` or `'edit'`), shown in the corrected example above.
:::
...
4. Render your provider under the main application provider, `\frontend\src\app\app-provider.tsx`, see below:

:::note
Page layout (sidebar/header/footer) is no longer wrapped inside `app-provider.tsx`. It is applied by `src/app/(main)/layout.tsx` using `useLayoutSelection`, so `app-provider.tsx` only needs to wrap `children` with your own providers, as shown below.
:::

---
sidebar_label: Dynamic Form Pattern
---
...
:::note
`FormConfiguration` now lives in the `Shesha.Domain` namespace, not `Shesha.Web.FormsDesigner.Domain`. Import `Shesha.Domain` when referencing it.
:::
...
:::warning Form Configurations table renamed
The `FormConfiguration` entity is now backed by the schema-qualified table `frwk.form_configurations` (lowercase), not `Frwk_FormConfigurations`. The `WithForeignKeyColumn(column, table)` helper used below does not accept a schema, so a foreign key to `FormConfiguration` must be added with the schema-aware `.ForeignKey(name, schema, table, column)` call shown below instead.
:::
...
{
    /// <summary>
    /// Creates the tables used by the dynamic form pattern tutorial
    /// </summary>
    ...
    public class M20240313180500 : Migration
    {
        ...
        public override void Up()
        {
            Create.Table("Tutorial_Templates")
                ...
                .WithColumn("CaptureFormId").AsGuid().Nullable()
                    .ForeignKey("FK_Tutorial_Templates_CaptureFormId", "frwk", "form_configurations", "id").Indexed()
...
Create [Dynamic Menu Items](./dynamic-menu-items.md) that will list all available templates and assist in initializing the member `Enquiry` on the `member-details` view.

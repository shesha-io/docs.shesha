---
sidebar_label: Custom Configurable Actions
---
...
export const useShowStaticMessageAction = () => {
    useConfigurableAction({
        ...
        hasArguments: false,
        executer: () => {
            actions.showMessage.info('This is a static message');
...
- **useConfigurableAction**: This hook registers the action, with the following parameters:
    ...
    - **ownerUid**: A unique id for the owner. Registration is silently skipped if either `owner` or `ownerUid` is blank, so both are effectively required.
    ...
    - **executer**: This function is the core logic for the action, which in this case displays a static message using `actions.showMessage.info`. It returns a `Promise` that resolves once the action is executed.
...
export const useShowDynamicMessageAction = () => {
    useConfigurableAction<IDynamicMessage>({
        ...
        executer: (actionArgs, context) => {
            ...
            actions.showMessage.info(`Hello ${user.userName}, ${dynamicMessage}`);
            return Promise.resolve();
        }
    }, []);
};
...
- The executer then displays a personalized message using actions.showMessage.info and resolves the Promise.

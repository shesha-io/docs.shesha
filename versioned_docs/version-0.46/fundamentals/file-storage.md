...
| Setting | Description |
|---|---|
| `ConnectionStrings:BlobStorage` | The Azure Storage connection string. Used if `CloudStorage:ConnectionString` is not set. |
| `CloudStorage:ConnectionString` | Optional. Takes priority over `ConnectionStrings:BlobStorage` when both are set. See [Connecting with a SAS URL](#connecting-with-a-sas-url) below. |
...
When a classic connection string (account name and key, or the Azurite emulator) is used, the blob container is created automatically if it does not exist, with public blob access.

#### Connecting with a SAS URL

Instead of a classic account-key connection string, `CloudStorage:ConnectionString` (or `ConnectionStrings:BlobStorage`) can be set to a SAS (Shared Access Signature) URL, for example `https://<account>.blob.core.windows.net/<container>?sv=...&sig=...`. Shesha detects this automatically because the value starts with `https://`.

- If the URL already includes a container in its path, that container is used directly and the configured `CloudStorage:ContainerName` is ignored.
- If the URL is an account-level SAS (no container in the path), it is combined with `CloudStorage:ContainerName`.

:::note
A SAS URL must include a query string (the SAS token itself, e.g. `?sv=...&sig=...`). A bare URL with no token throws an error on startup.
:::

...
:::warning CategoryField and CategoryValue have been removed
Earlier versions of this attribute exposed a `CategoryField` and `CategoryValue` pair to filter the related generic entities by category. Both properties have been removed from `DisplayGenericEntitesAuditTrailAttribute` as of this release, and the framework's audit lookup no longer filters by category. If you need to show only a subset of related generic entities, filter them another way, for example by giving the owning entity a dedicated relationship instead of relying on a shared generic reference.
:::

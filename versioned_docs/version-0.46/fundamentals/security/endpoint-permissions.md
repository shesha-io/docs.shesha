...
:::note
`[AllowAnonymous]` and `[AbpAllowAnonymous]` are checked directly on the request path and skip authorization entirely - no Permissioned Objects lookup happens for these endpoints at all. `[AbpAuthorize]` and `[AbpMvcAuthorize]` work differently at runtime: the request is still checked against the endpoint's Permissioned Objects record (see [Priority Summary](#priority-summary) below). What "hardcoded" means for these two is that the record is re-written from the code attribute on every startup, so it can never permanently drift from the code - not that the record is skipped at request time.
:::
...
1. `[AllowAnonymous]` or `[AbpAllowAnonymous]` on the type or method short-circuits authorization immediately - no Permissioned Objects lookup happens at all.
2. Otherwise, Shesha looks up the endpoint's Permissioned Objects record (this is where `[AbpAuthorize]`/`[AbpMvcAuthorize]`-hardcoded endpoints are actually enforced too, since their record is kept locked to the code value on every startup). If that record has an explicit (non-`Inherited`) access level, that value is used.
3. If the record is `Inherited`, Shesha walks up the parent service hierarchy.
4. If the parent hierarchy is fully `Inherited`, the `DefaultEndpointAccess` system setting is used as the final fallback - along with `DefaultEndpointPermissions` if the default access is `Requires Permissions`.

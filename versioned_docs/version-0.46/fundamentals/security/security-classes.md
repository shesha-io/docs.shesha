...
:::note What changed
The two parameters at the end of `AuthorizeAsync` used to be a single `RefListPermissionedAccess? replaceInherited` parameter. They are now `RefListPermissionedAccess? defaultAccess` and `List<string>? defaultPermissions`. The extra parameter matters: when the object's own access level is `Inherited` and the caller-supplied `defaultAccess` is `RequiresPermissions`, the permission list used for the check comes from `defaultPermissions` rather than the object's own (empty, since it's inherited) `ActualPermissions`. The caller of this method - `ApiAuthorizationHelper` - passes the `DefaultEndpointAccess` and `DefaultEndpointPermissions` security settings for these two parameters, so a system-wide default permission list can now apply to endpoints that are still set to `Inherited`.
:::

---

## `IPasswordComplexitySettings` and `ISecuritySettings`

Password complexity and general security settings are exposed as strongly-typed setting accessor interfaces rather than loose configuration keys. `IPasswordComplexitySettings` (category "Password complexity") exposes `RequireDigit`, `RequireLowercase`, `RequireNonAlphanumeric`, `RequireUppercase`, and `RequiredLength`, each backed by an ABP Zero setting name and marked `IsClientSpecific = true`, meaning each registered front-end can have its own values. `ISecuritySettings` exposes the compound `SecuritySettings` object described in [Authentication](./authentication.md#security-settings-general), plus `UserLockOutEnabled`, `DefaultAccountLockoutSeconds`, and `MaxFailedAccessAttemptsBeforeLockout` as their own settings.

...
Rather than forcing the user to log in again every time a token is close to expiring, the front end can call `POST /api/TokenAuth/RefreshTokenAsync` to exchange a still-valid token for a new one with a fresh expiration.

The endpoint blacklists the current token as part of the same call, before issuing the replacement, so a token can only be refreshed once - if the same token is submitted for a refresh a second time, the call fails because it has already been blacklisted. This closes off a replay window where a captured token could otherwise be refreshed repeatedly.

:::note
The refreshed token carries the user's current claims (re-built from the database at refresh time), not a copy of the claims on the old token. If the account has been deactivated since the original login, the refresh fails.
:::
...
|---|---|
| `UseAutoLogoff` | Whether Shesha automatically logs a user out after a period of inactivity. `AutoLogoffTimeout` below only takes effect while this is on |
...
These are all **General** settings - they apply once across the whole application rather than being repeated per front-end. The same form also holds [`DefaultEndpointAccess`](./endpoint-permissions.md), which is unrelated to authentication and covered on its own page.
...
Every setting in this table is **front-end specific**: it appears under a selected front-end (for example Default UI) rather than under the General view, so each registered front-end can enforce its own password and lockout policy. See [Settings Page Overview](../../settings/settings-overview.md) for how General and front-end-specific settings differ.
...
Shesha limits how often the login and OTP endpoints can be called from a single IP address. This shuts down password spraying, where an attacker tries one common password against many accounts, and OTP brute forcing, where they work through the possible pin codes. Limiting the OTP send endpoint also protects you from SMS toll fraud, where an attacker runs up your messaging bill by triggering thousands of sends.

The limits are applied per IP address using a fixed time window.
...
Only these two endpoints are limited. Ordinary application traffic, such as loading forms or working in the Configuration Studio, is never throttled.

A caller that exceeds the limit receives a `429 Too Many Requests` response with a `Retry-After` header giving the number of seconds to wait.

The limits and the retry period are configurable, so you can tighten or relax them to suit your environment.

:::warning Automated scripts will hit this
Load tests, integration test suites, and monitoring scripts that sign in repeatedly in quick succession start receiving `429` responses once they pass the limit. Reuse a token across requests rather than authenticating for each one, or raise the limit in the environment those scripts run against.
:::

:::note Shared IP addresses count as one caller
Because the limit is applied per IP address, users behind a single corporate gateway or NAT share a budget. If a large office reports intermittent login failures, the shared IP address is the first thing to check.
:::

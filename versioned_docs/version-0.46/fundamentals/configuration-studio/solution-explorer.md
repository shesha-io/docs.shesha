...
Clicking an item selects it on its own. To act on several items at once, for example to drag a group of forms into a different folder, select them together first.

| How | What it does |
|---|---|
| `Ctrl+click` (`Cmd+click` on Mac) | Adds the clicked item to the selection, or removes it if it was already selected |
| `Shift+click` | Selects every visible item between the last item you clicked and this one |
| `Shift+Up` or `Shift+Down` | Extends the selection one item at a time from the keyboard |

Once a group is selected you can drag it to a new module or folder in one move, rather than moving each item separately.

...
Items whose current revision holds changes you made by hand, rather than a revision that arrived in an earlier import, are marked in the export view. This makes it easy to see at a glance which items carry your own work when you are deciding what to include.
...
A package records the folder each item sits in, and the import recreates that structure in the target environment rather than dropping everything at the module root. Folders are matched by name and parent within the module, so the same layout resolves correctly across environments, and any folder in the package that does not exist yet is created.

Because the package is the source of truth for placement, importing an item that has since been moved in the target environment moves it back to where the package says it belongs. A move on its own does not create a new revision of the item's content.

:::note Packages exported before 0.46
Older packages do not record a folder path. Importing one leaves each item in whatever folder it is already in, so nothing is reorganised unexpectedly.
:::

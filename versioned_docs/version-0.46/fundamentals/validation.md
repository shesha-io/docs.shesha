...
|--|--|--|--|--|
| **Configuration** | Validation rules are specified without any changes to code through the [Entity Configurator](entity-Configuration), for example marking a property as required or read-only, or setting a minimum string length. | When simple validation rules supported by the configurator are required. | Limited by the validation scenarios supported by the configurator. |  |
...
Shesha uses the standard <a href="https://docs.fluentvalidation.net/en/latest/" target="_blank">FluentValidation</a> library through ABP's integration. Make sure the `AbpFluentValidationModule` is included as a dependency of the solution's `WebCoreModule` (or equivalent top-level module).
...
Write a validator by inheriting from `AbstractValidator<TEntity>` and register it against `IValidator<TEntity>` for that same entity type:
...
public class PersonValidator : AbstractValidator<Person>
{
    ...
    public PersonValidator(IRepository<Person, Guid> repository)
    {
        ...
        RuleFor(x => x.EmailAddress1).MustAsync(UniqueEmailAddress1Async).WithMessage("This email address is already in use");
    }

    private async Task<bool> UniqueEmailAddress1Async(Person person, string email, CancellationToken cancellationToken)
    {
        return !await _repository.GetAll().AnyAsync(p => p.Id != person.Id && p.EmailAddress1 == email);
    }
}
```

Register the validator against `IValidator<TEntity>` in your module's `Initialize()` method:
...
* For DynamicAPIs, any registered `IValidator<TEntity>` for the entity being created or updated is resolved and run automatically. If it fails, the Create (`POST`) or Update (`PUT`) API throws an `AbpValidationException` with the validation errors.
* For CustomAPIs, inject `IValidator<TEntity>` for the entity or DTO type you need to validate into your `AppService`, and call `ValidateAsync`.
...
public class AppService : SheshaAppServiceBase
{
   ...
   public AppService(IValidator<Person> validator)
   {
      _validator = validator;
   }

   [HttpPost, Route("CustomAPIMethod")]
   public async Task<CustomDto> CustomAPIMethod(Person input)
   {

...
|---|---|---|---|
| Get value | `POST` | `/api/services/app/Settings/GetUserValue` | `name`, `module`, `datatype` (optional), `defaultValue` (optional), `appKey` (optional) |
| Update value | `POST` | `/api/services/app/Settings/UpdateUserValue` | `name`, `module`, `value`, `datatype` (optional), `appKey` (optional) |

:::warning `GetUserValue` is a `POST` endpoint
Despite being a read operation, `GetUserValue` is invoked with `POST`, not `GET`. Both endpoints also use `name`/`module` (not `settingName`/`moduleName`) as their JSON body parameter names, and `UpdateUserValue` takes the new value in a `value` parameter, not `defaultValue`.
:::

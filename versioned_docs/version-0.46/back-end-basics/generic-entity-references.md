...
:::note
The attribute also exposes `IdColumnName`, `ClassNameColumnName`, and `DisplayNameColumnName` properties, which let you override the generated column names for a property if you need them to differ from the framework's default naming convention.
:::

### Step 2: Create the database migration

Add the `Id`, `ClassName`, and (optionally) `DisplayName` columns manually, following the naming convention above:
...
public class M20260227120000 : Migration
{
    public override void Up()
    {
        ...
        // Adds the RelatedEntityId, RelatedEntityClassName, and RelatedEntityDisplayName columns
        Alter.Table("MyApp_AuditEntries")
            ...
            .AddColumn("RelatedEntityDisplayName").AsString(1000).Nullable();
    }
...
If you don't need the display name column, omit the last `AddColumn` call:
...
:::warning `AddGenericEntityReferenceColumns` produces a mismatched column name
`Shesha.FluentMigrator.SheshaFluentMigratorExtensions` still exposes an `AddGenericEntityReferenceColumns(propertyName, storyDisplayName)` helper, but it names the class-name column `{propertyName}TypeName`, not `{propertyName}ClassName`. The entity mapping convention for `GenericEntityReference` properties (`Shesha.NHibernate.Maps.Conventions`) expects the column suffixed `ClassName` unless you override it with `ClassNameColumnName` on `[EntityReference]`. Using the extension method as-is will create a column NHibernate's default mapping won't find. The framework's own migrations (for example `StoredFile.Owner`) create the `Id`/`ClassName`/`DisplayName` columns manually, as shown above - do the same for your own properties rather than relying on this helper, unless you also set `ClassNameColumnName` on the attribute to match.
:::
...
If you don't have the entity loaded, you can construct the reference from its ID and type name. The constructor's parameters are `id`, `_className`, and an optional `_displayName`:
...
- [Domain Class Attributes](./domain-classes-attributes.md)

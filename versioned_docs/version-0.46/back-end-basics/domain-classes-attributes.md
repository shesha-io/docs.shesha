...
| `[Entity]`                                  | Provides parameters to add additional metadata to the entity and control additional aspects of the behavior of the entity: <br/> **`GenerateApplicationService`** - Specifies whether CRUD APIs for this entity should be generated. See also [CRUD APIs](./crud-apis.md). <br/> **`ApplicationServiceName`** - The name of the application service to be generated for the entity. This will be reflected in the URL of the dynamically generated CRUD API. If not specified, the name of the entity will be used. See also [CRUD APIs](./crud-apis.md). <br/> **`FriendlyName`** - A more user-friendly name for the entity to be used in the UI. If not specified, the name of the entity will be used. <br/> **`TypeShortAlias`** - (For internal use) Alias sometimes used by the framework instead of the fully qualified type name to identify the type of the entity. |
...
| **Attribute** | **Description** |
| --- | --- |
| `[Audited]`                                     | Identifies fields whose changes should be logged for easy tracking of when the change was made and the user responsible for that change. When used on the class level, all properties of that entity will be audited. See also [audit logging](../fundamentals/audit-logging.md). |
...
| `[ReferenceList("RefListName", optional moduleName)]` | Specifies the reference list that an `int` or `long` property is associated with. For more details, see [reference lists](./reference-lists.md). |

...
:::note
Both methods are `async`, so always `await` them before using the result - `GetCurrentUserAsync()` returns a `Task<User>`, not a `User`, until it is awaited.
:::
...
See the [Validation](../fundamentals/validation.md) section for more information on the various methods to defining validation rules.

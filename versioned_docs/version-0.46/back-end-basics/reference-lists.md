...
`ReferenceList` is a configuration item (it inherits from `ConfigurationItem`, the same base class described in [Custom Configuration Items](./custom-configuration-items.md)), so it can be managed the same way as forms or notification types.
...
:::warning `ReferenceListCreate` / `ReferenceListUpdate` / `ReferenceListDelete` are obsolete
`Shesha.FluentMigrator.SheshaExpressionRoot` still exposes `this.Shesha().ReferenceListCreate(...)`, `ReferenceListUpdate(...)`, and `ReferenceListDelete(...)`, but all three are now marked `[Obsolete]` in the framework source. They still work, and each now takes **two** arguments - `@namespace` then `name` - rather than the single `name` argument used previously. Prefer creating and maintaining data-based reference lists through the Configuration Studio where possible; use the migration API below only when you specifically need to seed or change reference list data as part of a migration.
:::
...
``` csharp
    ...
    public class M20250317111700 : Migration
    {
        public override void Up()
        {
            ...
            this.Shesha().ReferenceListCreate("MyApp", "Locations")
        ...
        {
            this.Shesha().ReferenceListDelete("MyApp", "Locations");
        }
    }
```
...
Beyond simply creating reference lists, Shesha provides a set of methods to manage data-based reference lists through database migration classes. These methods allow you to create, update, and delete reference list items programmatically as illustrated below. As noted above, the `@namespace` argument (typically your module name) is required alongside the reference list `name`.
...
this.Shesha().ReferenceListUpdate("MyApp", "Locations")
...
:::warning Multi-value reference list helper extensions
The `GetMultiValueReferenceListItemNames` and `GetMultiValueReferenceListItemNamesAr` extension methods on `Shesha.Extensions`, described in earlier versions, no longer exist in the framework.

To decompose a multi-value reference list value into its selected items, inject `IReferenceListHelper` (`Shesha.Services`) and call `DecomposeMultiValueIntoItems(refListId, value)`, which returns a `List<ReferenceListItemDto>` for the flags set on `value`:
...
public class MyService
{
    ...
    public MyService(IReferenceListHelper refListHelper)
    {
        _refListHelper = refListHelper;
    }

    public List<ReferenceListItemDto> GetSelectedDays(RefListDaysOfTheWeek? daysOpen)
    {
        var refListId = new ReferenceListIdentifier("MyApp", "DaysOfTheWeek");
        return _refListHelper.DecomposeMultiValueIntoItems(refListId, (long?)daysOpen);
    }
}
```
:::

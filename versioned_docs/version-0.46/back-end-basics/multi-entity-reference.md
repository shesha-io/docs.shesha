---
sidebar_label: Multi Entity Reference
sidebar_position: 4
---
...
The `ManyToManyAttribute` has several constructors that allow you to configure mapping and auto generation of tables:
...
- **table** - This defines the name of the link table that will be used for references.
...
- **where** - An optional SQL filter applied to the link table.
- **autoGeneration** - This indicates the table will be generated automatically based on the names defined in the attribute, or automatically generated names will be used if not defined.
...
If you don't want to create the table manually, you can use the `autoGeneration` parameter of the `ManyToMany` attribute:
...
| `AuditedAsManyToMany` | Saves changes for the parent entity and for the child entities. |

**Example for `Audited` on organization *Main office*:**

| EventName | Description |
|---|---|
| *Employees* updated | *Employees* updated. Added: John Doe. Removed: Jane Doe. |

**Example for `AuditedAsManyToMany`:**

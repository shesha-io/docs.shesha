...
- **.NET 10 SDK & Runtime** – Needed for the backend (ASP.NET Core) and for SQL-Package to work.
- **Node.js 24** – Required to run the Shesha frontend (React.js). npm 11 or later is also required.
...
:::tip Running commands with elevated privileges
Some commands need administrator privileges. Prefix them with `sudo`. On Windows you may need to enable `sudo` in your settings first, if it is not already available in your shell.
:::
>
> ```bash
> sudo ...
...
:::note HTTPS development certificate
If you do not have a local development certificate, install and trust one:
:::
> ```bash
...
The frontend connects to the backend on start-up, so make sure the backend is running before you start the frontend.
:::

...
:::note A similar built-in endpoint already exists
Shesha's built-in `UserManagementAppService.CreateAsync` (the endpoint behind `/api/services/app/UserManagement/Create`, see [Frontend Implementation](#frontend-implementation) below) already performs very similar validation and user-creation logic out of the box. Only write a custom app service like the one below if you need extra validation or extra properties that the built-in endpoint does not cover.
:::

:::warning `UserManager.CreateUser` has been renamed
`UserManager.CreateUser(...)` is no longer available. Use the async `UserManager.CreateUserAsync(...)` method shown below instead. Its second parameter, `createLocalPassword`, replaces the old `isUsingSql` parameter, and it now also takes a required `requireChangePassword` parameter (with an optional `supportedPasswordResetMethods` parameter after it). The example below is also updated for `RefListTypeOfAccount`, which only defines `External` and `Internal` members - there is no `SQL` member.
:::
...
/// </summary>
[SheshaAuthorize(RefListPermissionedAccess.RequiresPermissions, "YourApp.UserManagement")]
[ApiVersion("1")]
...
public class HealthPractitionerAppService : DynamicCrudAppService<HisPractitioner, DynamicDto<HealthPractitioner, Guid>, Guid>, ITransientDependency
{
    ...
    [SheshaAuthorize(RefListPermissionedAccess.RequiresPermissions, "YourApp.UserManagement.Create")]
    public async Task<DynamicDto<HisPractitioner, Guid>> CreateWithUserAccountAsync(CreatePersonAccountDto input)
    {
        ...
        User user = await userManager.CreateUserAsync(
                input.UserName,

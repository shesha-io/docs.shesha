---
sidebar_label: Configure an API Call Action
title: How to Configure an "API Call" Action (Step-by-Step)
---
...
![Selecting "Common: API Call" from the action dropdown](./images/api-call-action/01-select-api-call-action.png)
...
![HTTP Verb set to POST and URL set to the dynamic CRUD Create endpoint](./images/api-call-action/02-http-verb-and-url.png)
...
![Adding key/value parameters on the Params tab of the Configure Request dialog](./images/api-call-action/03-configure-request-params.png)
...
- **Calling an external API with its own auth scheme.** For example, an
  ...
  instead you add the header the external service actually requires:
    ...
    ![A custom API-key header configured for an external service](./images/api-call-action/04-external-api-key-header.png)
...
- **Calling your own backend without the current user's session** — e.g. from
  ...
  still requires a token (a service account token, for instance):
    ...
    ![A service-account Bearer token header configured for an anonymous form](./images/api-call-action/05-service-account-bearer-header.png)
...
- **JSON** — write the payload directly in a JSON editor. Values can be
  hard-coded or reference form data via a Mustache expression, e.g.
    ...
    ![A JSON body using a Mustache expression to reference form data](./images/api-call-action/06-body-json-mustache.png)
...
- **form-data** — instead of hand-writing JSON, add fields one at a time via
  **Add Field**. The **Key** column autocompletes against your form's data
  ...
  either a literal value or a `{{expression}}` referencing form data:
    ...
    ![form-data fields with key autocomplete against form data properties](./images/api-call-action/07-body-form-data-fields.png)
    ...
    ![The Include checkbox toggling individual form-data fields on and off](./images/api-call-action/08-body-form-data-include-toggle.png)
...
![An x-www-form-urlencoded body configured against httpbin.org/post](./images/api-call-action/09-body-urlencoded.png)

![httpbin's response echoing back the URL-encoded form fields](./images/api-call-action/10-body-urlencoded-httpbin-response.png)

### Understanding the `raw` body type

![The raw body type with its sub-type selector](./images/api-call-action/11-body-raw-subtype-selector.png)
...
- **`text` / `json` / `xml` / `html`** — the content is a **static template**.
  ...
  parts that don't map cleanly to fields.
    ...
    **Try it yourself:** HTTP Verb **POST**, URL `https://httpbin.org/post`,
    ...
    ![POST verb and httpbin.org/post URL configured for the raw body test](./images/api-call-action/12-raw-xml-verb-and-url.png)

    ![A raw XML body containing a Mustache expression](./images/api-call-action/13-raw-xml-body.png)

    ![httpbin's response showing the substituted XML and the automatic Content-Type](./images/api-call-action/14-raw-xml-httpbin-response.png)
...
- **`javascript`** — the content isn't a template at all, it's a **script**.
  ...
  that string is sent as-is.
    ...
    **Try it yourself:** same URL/verb as above, sub-type **javascript**:
    ...
    ![A javascript raw body whose return value becomes the request payload](./images/api-call-action/15-raw-javascript-body.png)

    ![httpbin's response showing the object returned by the script as JSON](./images/api-call-action/16-raw-javascript-httpbin-response.png)
...
1. Write a script that both derives an email address *and* writes it
   straight onto the visible `emailAddress1` field:
    ...
    ![The Transformation tab with a script deriving an email address and calling form.setFieldValue](./images/api-call-action/17-transformation-script.png)
...
![The form field updated and the derived value available on actionResponse](./images/api-call-action/18-transformation-result.png)

---
sidebar_label: Flattened Entities for Performance
title: Improving Table View Performance With Flattened Entities
---
...
The "N+1 selects" problem describes the situation where loading one main record (the +1) triggers one additional database query for each related record it links to (the N). With many rows on a table view, this multiplies into hundreds of small queries. See [this Stack Overflow answer](https://stackoverflow.com/questions/97197/what-is-the-n1-selects-problem-in-orm-object-relational-mapping) for the full background.
:::

:::warning Flattened entities are read-only
Because flattened entities map onto database views rather than tables, they cannot be used for create or update operations. For those, use the original "unflattened" version of the entity.
:::
...
Create a database view that joins in the related tables so the result set contains every column the table view needs to display. Wrap the view definition in a FluentMigrator migration so it can be deployed consistently to every environment.

The recommended naming convention is `vw_{Module DB Prefix}_Flattened{Entity name plural}`, for example `vw_His_FlattenedAppointments`.
...
Use the single-parameter form `[ReferenceList("Name")]`. The two-parameter form that accepts a namespace separately is marked obsolete.
:::
...
Once the entity is in place, bind to it from a Table View like any other entity. The table view now loads all of its data through a single database call against the view, rather than one call per related record.

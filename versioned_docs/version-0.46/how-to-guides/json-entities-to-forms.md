...
public class JsonSpoke : JsonEntity
{
...
When the user picks a row in the table, `form.components.DataTableContext1.selectedRow` updates. This changes the SubForm's Query Params, which loads the JSON data for the selected `TestClass`. The SubForm reads `_className` off the loaded data and swaps to the matching detail form automatically.

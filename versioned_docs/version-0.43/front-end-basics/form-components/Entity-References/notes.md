...
The type of entity that owns these notes, for example `Shesha.Domain.Person`. This setting is required, and is picked using an autocomplete populated from the entity metadata API.
...
Fires after an existing note is successfully updated. In addition to the shared variables listed under On Create, this event exposes the updated note as `event`:

| Variable | Type | Description |
|---|---|---|
| `event` | `object` | The updated note, including its `id`, `noteText`, `author`, `category`, `priority`, and `creationTime`. |

:::note
The settings panel labels this variable "note" in its description, but the value is only available in your script as `event`, not `note`.
:::
...
```js
message.info(`Note updated: ${event.noteText}`);
```
...
Fires after a note is successfully deleted. In addition to the shared variables listed under On Create, this event exposes the deleted note as `event`, using the same shape described under On Update.
